import { DashboardRequestPayload, Message } from "./types.ts";

const PPLX_API_KEY = (globalThis as typeof globalThis)["Deno"]?.env.get(
  "PPLX_API_KEY",
);

async function callPerplexityAPI(systemPrompt: string, messages: Message[]) {
...
export async function handleDashboardRequest(payload: DashboardRequestPayload) {
  const {
    mode,
    query,
    chatHistory,
  ...
  let formattedPrompt = systemPrompt
    ...
    .replace(
      "{{structuredDataSource}}",
      JSON.stringify(structuredDataSource || "NONE", null, 2),
    );
  ...
  if (mode === "r1" || mode === "r2") {
    formattedPrompt = formattedPrompt
      .replace(
        "{{previousWidgetsJSON}}",
        JSON.stringify(previousWidgetsJSON || "NONE", null, 2),
      )
      .replace(
        "{{chatHistory}}",
        JSON.stringify(chatHistory || "NONE", null, 2),
      );
  }
  ...
  if (mode === "r2") {
    formattedPrompt = formattedPrompt
      .replace(
        "{{focusedWidgetsData}}",
        JSON.stringify(focusedWidgetsData || "NONE", null, 2),
      );
  }
...
const systemPrompts = {
  ...
  l1:
    `Context: You are a real-time dashboard generator for a lightweight embedded system that uses Apache ECharts in a React app. The user gives a natural language prompt, and your job is to generate a minimal, valid, structured JSON response that can be rendered immediately by the frontend.
  ...
  r1:
    `Context: You are modifying or regenerating an existing dashboard based on a follow-up prompt from the user. The system supports complex interactive visualizations using Apache ECharts and other widget types.
  ...
  r2:
    `Context: The user wants to modify specific widget(s) from their dashboard. Your job is to return the full dashboard with ONLY the targeted widgets modified.
