...
import { useWidgetTheme, WidgetThemePreset, themePresets } from '../widgets/util/chartThemeUtil';
...
const DashboardThemeSelector: React.FC = () => {
  ...
  const ThemeCircle = ({ preset, size = 32, onClick }: { preset: WidgetThemePreset, size?: number, onClick?: () => void }) => {
  ...
  return (
    <Box>
      <Tooltip title="Widget Theme">
        <IconButton onClick={handleClick} size="small" sx={{ p: 0.5 }} key={`theme-selector-button-${themePreset}`}>
