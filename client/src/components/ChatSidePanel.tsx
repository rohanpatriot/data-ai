...
const ChatSidePanel: React.FC<ChatSidePanelProps> = ({ setChatOpen, messages, newMessage, 
    setNewMessage, handleSendMessage, setShowDataSourcesModal
 }) => {
  return (
    <Box sx={{ 
        width: '330px', 
        ...
        <Box sx={{ p: 1.5, borderBottom: '1px solid #eaeaea', display: 'flex', alignItems: 'center' }}>
            <Logo />
            <IconButton 
        ...
        <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 2 }}>
            ...
            <Box key={index} sx={{ mb: 3, display: 'flex', alignItems: 'flex-start' }}>
                {msg.sender === 'user' ? (
                <Avatar src='/src/assets/dev/avatar.svg'
                sx={{ mr: 2, width: 36, height: 36 }}
                />
                ) : (
                <Avatar 
                    src="/src/assets/brandmark.svg" 
                    sx={{ mr: 2, width: 36, height: 36, bgcolor: 'transparent', padding: 0.8, border: '1px solid hsl(var(--primary))' }} 
                />
                )}
                <Box sx={{ maxWidth: '85%' }}>
                ...
                <Typography variant="body2" sx={{textAlign: 'start' }}>{msg.text}</Typography>
                </Box>
            </Box>
            ))}
        </Box>
